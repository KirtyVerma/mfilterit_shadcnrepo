...
interface CardSwitchProps {
  ...
  Sub_title?: string[];
  Placeholder?: string;
  value?: { title: string, options: string[] }[];
  isSelect?: boolean;
  direction?: "rows" | "cols";
  isLoading?: boolean;
  enabledStates?: boolean[];  // Add this new prop
  onSwitchChange?: (index: number, checked: boolean) => void;  // Add this new prop
  loadingIndex?: number;
}
...
const CardwithSwitch: React.FC<CardSwitchProps> = ({
  ...
  direction = "cols",
  isLoading = false,
  enabledStates = [],  // Add default value
  onSwitchChange,     // Add this prop
  loadingIndex,
}) => {
  const contentRef = useRef<HTMLDivElement>(null);
  const [contentHeight, setContentHeight] = useState<number>(300); // Set default minimum height

  useEffect(() => {
    const updateHeight = () => {
      if (contentRef.current) {
        const height = Math.max(contentRef.current.scrollHeight, 300); // Ensure minimum height
  ...
  return (
    <Card className='border-none w-full h-full p-2'>
      ...
      <CardContent 
        className="relative min-h-[200px]" // Add minimum height
      >
        {isLoading ? (
                ...
                <div className="flex flex-row items-center w-full">
                  ...
                  {isSelect && (
                    <Select>
                      ...
                      <SelectContent>
                        <SelectGroup>
                          {value.map((value) => (value.title === title &&
